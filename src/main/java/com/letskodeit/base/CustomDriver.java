...
public class CustomDriver {
    ...
    public boolean verifyCurrentURL(String expURL){
        boolean flag = false;
        if (expURL.equalsIgnoreCase(getURL())) {
            flag = true;
        }else{
            flag = false;
        }
    ...
    public boolean verifySearchedResultsBasedOnSearchTerm(String locator, String searchTerm){
        String final_Locator = Util.stringFormatter(locator,searchTerm);
        System.out.println("final_Locator: "+ final_Locator );
        boolean flag = false;
        List<WebElement> results = getElementList
                (final_Locator,"Searched Results based on search term: "+ searchTerm);
        if(results.size() > 0){
